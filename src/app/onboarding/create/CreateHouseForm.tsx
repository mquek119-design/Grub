...
/** Aldi-inspired logo — adapted */
function AldiInspiredLogo({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 36 36" className={className} xmlns="http://www.w3.org/2000/svg">
      {/* Shield shape */}
      <path
        d="M18 4L6 10v10c0 7 5 12 12 12s12-5 12-12V10L18 4z"
        fill="#061E42"
        stroke="#F5A623"
        strokeWidth="1.5"
      />
      {/* Inner triangle */}
      <path d="M18 10l-6 12h12L18 10z" fill="#F5A623" opacity="0.9" />
      {/* Stripe */}
      <rect x="12" y="24" width="12" height="2" rx="1" fill="#D63939" />
    </svg>
  );
}
              ...
              {SUPERMARKETS.map((market) => {
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      'p-4 rounded-2xl border text-left flex items-center gap-4 transition-all btn-tactile cursor-pointer',
                      isSelected
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-title-sm text-sm font-bold text-on-surface leading-tight">{market.name}</p>
                      <p className="font-body-xs text-[11px] text-on-surface-variant mt-0.5">{market.location}</p>
                    </div>
                    <span className={clsx(
